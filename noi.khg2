-- [[ KRAISORN HUB V.40: P-MENU EDITION ]]
-- OWNER: ไกรสร พิสิษฐ์ 🫡
-- KEY: 1
...
keyInput.Size = UDim2.new(0, 220, 0, 35); keyInput.Position = UDim2.new(0.5, -110, 0.45, 0); keyInput.PlaceholderText = "ใส่รหัสคีย์..."; keyInput.Text = ""; keyInput.BackgroundColor3 = Color3.fromRGB(45, 45, 45); keyInput.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", keyInput)

local submitBtn = Instance.new("TextButton", keyFrame)
submitBtn.Size = UDim2.new(0, 220, 0, 35); submitBtn.Position = UDim2.new(0.5, -110, 0.75, 0); submitBtn.Text = "ตรวจสอบ"; submitBtn.BackgroundColor3 = Color3.fromRGB(100, 100, 255); submitBtn.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", submitBtn)

---------------------------------------------------------
-- [ เมนูหลัก (หลังใส่คีย์) ]
---------------------------------------------------------
local function BuildMainMenu()
    ...
    menuFrame.Size = UDim2.new(0, 250, 0, 320); menuFrame.Position = UDim2.new(0.12, 0, 0.4, 0)
    ...
    scroll.Size = UDim2.new(1, 0, 1, -20); scroll.Position = UDim2.new(0, 0, 0, 10); scroll.BackgroundTransparency = 1; scroll.CanvasSize = UDim2.new(0, 0, 0, 450); scroll.ScrollBarThickness = 2
    ...
    -- 1. Auto Eat (ฟังก์ชันใหม่ที่คุณต้องการ)
    local autoEatEnabled = false
    local eatBtn = createMenuBtn("Auto Eat: ปิด", Color3.fromRGB(100, 100, 100), function(b)
        autoEatEnabled = not autoEatEnabled
        if autoEatEnabled then
            b.Text = "Auto Eat: เปิด"
            b.BackgroundColor3 = Color3.fromRGB(0, 170, 0) -- สีเขียวเข้มตอนเปิด
        else
            b.Text = "Auto Eat: ปิด"
            b.BackgroundColor3 = Color3.fromRGB(100, 100, 100) -- สีเทาตอนปิด
        end
    end)

    -- Loop สำหรับ Auto Eat (รันทุก 0.1 วิ)
    task.spawn(function()
        while true do
            if autoEatEnabled then
                local event = ReplicatedStorage:WaitForChild("EatingHandler_holdFoodEvent", 5)
                if event then
                    event:FireServer(Vector3.new(-64.5, 6.063042640686035, -63.96592712402344))
                end
            end
            task.wait(0.1)
        end
    ...
    createMenuBtn("HIT 1 (Red)", Color3.fromRGB(255, 50, 50), function(b)
        ...
        for i = 5, 1, -1 do b.Text = "COOLING: "..i; task.wait(1) end
        b.Text = "HIT 1 (Red)"; cd1 = false
    end)

    -- 3. H2
    local cd2, clicks = false, 0
    createMenuBtn("HIT 2 (5)", Color3.fromRGB(80, 255, 80), function(b)
        if cd2 then return end
        ReplicatedStorage.Events.Hit:FireServer(0.2337765097618103)
        clicks = clicks + 1; b.Text = "HIT 2 (".. 5-clicks ..")"
        if clicks >= 5 then
            cd2 = true; for i = 4, 1, -1 do b.Text = "WAIT: "..i; task.wait(1) end
            clicks = 0; b.Text = "HIT 2 (5)"; cd2 = false
        end
    end)
    ...
    createMenuBtn("CHAIR CRATE", Color3.fromRGB(50, 100, 255), function(b)
        ...
        local t = 1.5; while t > 0 do b.Text = string.format("CD: %.1f", t); task.wait(0.1); t = t - 0.1 end
        b.Text = "CHAIR CRATE"; cd3 = false
    end)
    ...
    createMenuBtn("SLAP CRATE", Color3.fromRGB(255, 220, 50), function(b)
        ...
        b.Text = "CD: 1s"; task.wait(1)
        b.Text = "SLAP CRATE"; cd4 = false
    end)

    -- ระบบลากปุ่ม P และเปิดเมนู
    ...
    pBtn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true; dragStart = input.Position; startPos = pBtn.Position
        end
    end)
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - dragStart
            pBtn.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
            if menuFrame.Visible then menuFrame.Position = UDim2.new(pBtn.Position.X.Scale, pBtn.Position.X.Offset + 80, pBtn.Position.Y.Scale, pBtn.Position.Y.Offset) end
        end
    end)
...
submitBtn.MouseButton1Click:Connect(function()
    if keyInput.Text == "1" then
        ...
        submitBtn.Text = "คีย์ผิด!"; task.wait(1); submitBtn.Text = "ตรวจสอบ"
    end
end)
