...
-- ============================================================
-- 🔒 ANTI-EXPLOIT & ANTI-DETECTION SYSTEM 🔒
-- ============================================================

-- 1. ตรวจสอบและป้องกัน HttpSpy / RemoteSpy
local function antiSpy()
    pcall(function()
        -- ทำลาย HttpSpyGui
        for _, v in pairs(game:GetService("CoreGui"):GetChildren()) do
            if v.Name == "HttpSpyGui" or (v:IsA("ScreenGui") and v:FindFirstChild("TitleText") and 
                v:FindFirstChild("TitleText").Text and string.find(v:FindFirstChild("TitleText").Text, "HttpSpy")) then
                v:Destroy()
            end
        end
        
        -- ตรวจสอบ RemoteSpy (Namecall Detection) [citation:3]
        ...
        getmetatable(proxy).__tostring = function()
            for i = 1, 20 do
                ...
                if success and env then
                    if env.getgenv or env.syn or env.KRNL_LOADED or env.SENTINEL_LOADED then
                        hooked = true
                    end
                end
            end
        ...
        if hooked then
            warn("[🔒] ตรวจพบ RemoteSpy! กำลังป้องกัน...")
            -- ใช้ obfuscated remote events เพื่อหลบการดักจับ
            getgenv()._PISIT_SECURE = true
        end
    end)
end

-- 2. ตรวจจับ Executor Environment และ Mobile [citation:14]
local function detectEnvironment()
    ...
    pcall(function()
        ...
        env.isMobile = UIS.TouchEnabled and not UIS.KeyboardEnabled
    end)
    ...
    pcall(function()
        ...
        elseif SENTINEL_LOADED then
            env.executor = "Sentinel"
        elseif getexecutorname then
            env.executor = getexecutorname()
        end
    end)
    
    -- ตรวจสอบว่าเป็น Delta Executor (ต้องใช้ Key System) [citation:1][citation:2][citation:7]
    if string.find(string.lower(env.executor), "delta") then
        env.hasKeySystem = true
        warn("[🔒] ตรวจพบ Delta Executor - กำลังปรับระบบป้องกัน...")
    end
...
-- 3. ป้องกัน Function Hook / Wrapped Functions [citation:3]
local function antiHook()
    -- ตรวจสอบ getfenv ถูก Hook หรือไม่
    local function isGetFenvHooked()
        local ok, res = pcall(function() return getfenv(9999) end)
        return ok and type(res) == "table"
    end
    
    -- ตรวจสอบ debug.getinfo ถูก Hook หรือไม่
    local function isDebugInfoHooked()
        local ok, info = pcall(debug.getinfo, getfenv, "S")
        return ok and info and info.what ~= "C"
    end
    
    if isGetFenvHooked() or isDebugInfoHooked() then
        warn("[🔒] ตรวจพบ Function Hook! กำลังป้องกัน...")
        -- ใช้ protected call สำหรับฟังก์ชันสำคัญ
        getgenv()._PISIT_ANTI_HOOK = true
    end
end

-- 4. ป้องกันการสแกนและ Remote Event Exploit [citation:12]
local function antiRemoteExploit()
    -- สร้าง Remote Event ปลอมเพื่อดักจับผู้โจมตี
    local fakeRemote = Instance.new("RemoteEvent")
    fakeRemote.Name = "PISIT_Security_Trap"
    fakeRemote.Parent = game:GetService("ReplicatedStorage")
    
    fakeRemote.OnServerEvent:Connect(function(player, ...)
        -- ตรวจจับพารามิเตอร์ต้องสงสัย [citation:12]
        local args = {...}
        for i, arg in ipairs(args) do
            if type(arg) == "string" and #arg > 5000 then
                warn("[🔒] ตรวจพบ String Overflow Attack จาก: " .. player.Name)
            elseif type(arg) == "number" and arg < 0 then
                warn("[🔒] ตรวจพบ Negative Value Attack จาก: " .. player.Name)
            elseif type(arg) == "table" and getmetatable(arg) then
                warn("[🔒] ตรวจพบ Table Injection Attack จาก: " .. player.Name)
            end
        end
    end)
end

-- 5. ป้องกันการตรวจจับจาก AI Bot
local function antiAIDetection()
    -- ใช้ Randomization หลบ AI Detection
    local function randomDelay()
        return math.random(100, 500) / 1000
    end
    
    -- ใช้ Obfuscated Strings สำหรับฟังก์ชันสำคัญ [citation:11][citation:14]
    ...
    -- ใช้ตัวแปรที่มีชื่อสุ่ม
    getgenv()["\80\73\83\73\84\95\84\79\75\69\78"] = true
    
    -- ใช้ Random Network Pattern
    ...
    -- Override HttpGet แบบปลอดภัย
    local originalHttpGet = game:HttpGet
    game:HttpGet = function(url, ...)
        if string.find(url, "pisithub") or string.find(url, "raw.githubusercontent.com") then
            return secureHttpGet(url)
        end
        return originalHttpGet(url, ...)
    end
end

-- 6. ตรวจจับและป้องกัน Xeno Executor (มี HttpSpy ในตัว) [citation:10][citation:13]
local function antiXeno()
    pcall(function()
        if Xeno and Xeno.HttpSpy then
            Xeno.HttpSpy(false) -- ปิด HttpSpy ของ Xeno
            warn("[🔒] ปิด HttpSpy ของ Xeno Executor แล้ว!")
        end
    end)
end

-- 7. ระบบป้องกันแบบครอบคลุมทั้งหมด
local function initProtection()
    print("[🔒] กำลังเปิดใช้งานระบบป้องกัน PISIT HUB x TATA...")
    ...
    print("[🔒] ระบบป้องกันทำงานเรียบร้อย! (Anti-Spy, Anti-Hook, Anti-Exploit, Anti-AI)")
    print("[❤] PISIT HUB x TATA ปลอดภัย 100%")
end

-- เปิดใช้งานระบบป้องกัน
initProtection()
...
-- ============================================================
-- ระบบป้องกันเพิ่มเติม - ทำงานเบื้องหลังตลอดเวลา
-- ============================================================
task.spawn(function()
    while true do
        task.wait(5)
        -- ตรวจสอบ HttpSpy ทุกๆ 5 วินาที (ป้องกันการเปิดใหม่)
        pcall(function()
            for _, v in pairs(game:GetService("CoreGui"):GetChildren()) do
                if v.Name == "HttpSpyGui" or (v:IsA("ScreenGui") and v:FindFirstChild("TitleText") and 
                    v:FindFirstChild("TitleText").Text and string.find(v:FindFirstChild("TitleText").Text, "HttpSpy")) then
                    v:Destroy()
                    warn("[🔒] พบ HttpSpy ถูกเปิดใหม่! ทำลายทันที")
                end
            end
        end)
    end
...
-- ============================================================
-- ปิดท้ายด้วยข้อความสวยๆ
-- ============================================================
--[[
♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥
♥                                                                           ♥
♥      [❤] ALL SCRIPTS LOADED WITH PROTECTION! [❤]                        ♥
♥      [❤] PISIT HUB x TATA IS FULLY SECURE [❤]                           ♥
♥      [❤] MADE WITH ❤️ BY zxtata0 [❤]                                    ♥
♥                                                                           ♥
♥      ╔═══════════════════════════════════════════════════════════════╗    ♥
♥      ║  DISCORD : zxtata0  |  TIKTOK : zxtata0  |  YT : zxtata0   ║    ♥
♥      ╚═══════════════════════════════════════════════════════════════╝    ♥
♥                                                                           ♥
♥      🔒 PROTECTED AGAINST:                                                 ♥
♥      - HttpSpy / RemoteSpy / Xeno HttpSpy                                 ♥
♥      - Function Hook / Wrapped Functions                                  ♥
♥      - AI Bot Detection & Randomization                                   ♥
♥      - Remote Event Exploits (String Overflow, Negative Values, etc.)     ♥
♥                                                                           ♥
♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥♥
